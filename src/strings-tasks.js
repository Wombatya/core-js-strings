...
function sumOfCodes(str) {
  ...
  const arr = str.split('');
  const newArr = arr.map((el) => el.charCodeAt()).reduce((acc, el) => acc + el);
  return newArr;
}
...
function isPalindrome(str) {
  ...
  if (str[str.length - 1] === '!' || str[str.length - 1] === '?') {
    compareStr = str.slice(0, str.length - 1);
  }
...
function encodeToRot13(str) {
  ...
  const result = [];
  newStr.forEach((el) => {
    const ind = abcNormal.indexOf(el);
    result.push(abcCoded[ind]);
  });
  return result.join('');
}
